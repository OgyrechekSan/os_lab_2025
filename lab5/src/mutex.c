...
int r1 = 0, r2 = 0, r3 = 0;
pthread_mutex_t mut = PTHREAD_MUTEX_INITIALIZER;//макрос для статической инициализации
...
void do_one_thing(int *pnum_times) {
  ...
  int work;
  for (i = 0; i < 50; i++) {
    pthread_mutex_lock(&mut);//ахват мьютекса (вход в критическую секцию),если он свободен,то захватывается потоком,иначе поток ждёт освобождения
